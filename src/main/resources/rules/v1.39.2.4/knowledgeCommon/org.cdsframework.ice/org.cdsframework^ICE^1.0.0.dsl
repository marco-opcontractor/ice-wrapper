...
[condition][]There is [Aa]n [Aa]dministered [Ss]hot {assign_oTargetDose} identified by {assign_oSameTargetDose}={assign_oTargetDose} : TargetDose(this == {assign_oSameTargetDose})
...
[condition][]- [Tt]he [Ss]hot dose is after than the shot the series was marked as complete=administeredShotNumberInSeries > getDoseAfterWhichSeriesWasMarkedComplete()
...
[condition][]- [Tt]he [Nn]ame of the [Ss]eries not a member of {list_sSeriesName:[\\(]+[a-zA-Z0-9\\.\\-_\\"\\,\\ \\(\\)]+[\\)]+}=seriesRules.seriesName not in {list_sSeriesName}
...
[condition][]- [Tt]his [Rr]ule {sRuleName} has not been executed before for this [Ss]eries=containsRuleProcessed({sRuleName}) == false
...
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Vv]alid due to "Special Rule"={refer_oTargetDose}.addValidReason("EVALUATION_REASON_CONCEPT.SPECIAL_RULE");
...
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Vaccine no longer recommended on date specified."={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.VACCINE_NO_LONGER_RECOMMENDED_ON_DATE_SPECIFIED");
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Vaccine Not Allowed Second Season RSV mAB"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.INVALID_SECOND_RSV_SEASON"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Vaccine Not Allowed Second Season RSV mAb clesrovimab"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.INVALID_CLESROVIMAB_SECOND_RSV_SEASON"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Vaccine Not Allowed First Season After Enflonsia"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.INVALID_RSV_MAB_SEQUENCE_FIRST_RSV_SEASON"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Second Dose Not Allowed in First Season"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.ONLY_ONE_VALID_DOSE_ALLOWED_FIRST_SEASON"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Third dose is not indicated for completed the MenB 4C 2-dose"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.SERIES_ALREADY_COMPLETE_MENB4C2DOSE"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
...
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Above maximum age for the vaccine"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.ABOVE_MAXIMUM_AGE_VACCINE"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
...
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Invalid due to Not Forecast Indicator"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.VACCINE_NOT_RECOMMENDED_NOT_FORECAST_INDICATOR"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
[consequence][][Ii]nclude the [Rr]eason for [Ss]hot {refer_oTargetDose} [Ii]nvalid due to "Outside Meningococcal Season with SDM"={refer_oTargetDose}.addInvalidReason("EVALUATION_REASON_CONCEPT.OUTSIDE_MENB_SDM_RANGE"); {refer_oTargetDose}.setRuleName(drools.getRule().getName());
...
[consequence][][Ss]et the [Ss]hots of the [Ss]eries {refer_oTargetSeries} after the dose number of {refer_oTargetDoseNumber} to accepted due to "Extra Dose"=for (TargetDose td : {refer_oTargetSeries}.getTargetDoses()) \{  if (td.getAdministeredShotNumberInSeries() > {refer_oTargetDoseNumber} && td.getStatus() == DoseStatus.VALID) \{ td.setStatus(DoseStatus.ACCEPTED); td.addAcceptedReason("EVALUATION_REASON_CONCEPT.EXTRA_DOSE"); \} \}
...
[consequence][][Cc]onvert the string date of {strDate:[\\"]{1}[0-9]+[\\-]{1}[a-zA-Z]+[\\-]{1}[0-9]+[\\"]{1}} into a date variable and save as {assign_dtDateCalculated}=Date {assign_dtDateCalculated} = TimePeriod.generateDateFromStringInDroolsDateFormat({strDate});
[consequence][][Cc]alculate the latter of {dtDate1} and {dtDate2} and save [Cc]alculated [Dd]ate as {assign_dtDateCalculated}=Date {assign_dtDateCalculated} = {dtDate2}; if({dtDate1}.after({dtDate2})) \{ {assign_dtDateCalculated} = {dtDate1}; \}
...
[consequence][][Ss]et the Final [Rr]ecommendation [Ee]arliest [Ff]orecast [Dd]ate on series {refer_oTargetSeries} to {dtForecastDate}={refer_oTargetSeries}.setFinalEarliestDate({dtForecastDate});
[consequence][][Ss]et the Final [Rr]ecommendation [Rr]ecommended [Ff]orecast [Dd]ate on series {refer_oTargetSeries} to {dtForecastDate}={refer_oTargetSeries}.setFinalRecommendationDate({dtForecastDate});
[consequence][][Ss]et the Final [Rr]ecommendation [Oo]verdue [Ff]orecast [Dd]ate on series {refer_oTargetSeries} to {dtForecastDate}={refer_oTargetSeries}.setFinalOverdueDate({dtForecastDate});
...
[consequence][][Ii]nclude a [Rr]ecommendation as {assign_oRecommendation} with [Ss]tatus {enum_RecommendationStatus} and [Rr]ecommended [Ff]orecast with [Dd]ate of {strDate:[\\"]{1}[0-9]+[\\-]{1}[a-zA-Z]+[\\-]{1}[0-9]+[\\"]{1}} for [Cc]onsideration in the final [Ff]orecast of the [Ss]eries {refer_oTargetSeries}=Recommendation {assign_oRecommendation} = new Recommendation({refer_oTargetSeries}); {assign_oRecommendation}.setRuleName(drools.getRule().getName()); {assign_oRecommendation}.setRecommendationDate(TimePeriod.generateDateFromStringInDroolsDateFormat({strDate})); {assign_oRecommendation}.setRecommendationStatus({enum_RecommendationStatus}); insert({assign_oRecommendation});
...
// Logging actions
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
[consequence][][Rr]ecord that the [Ss]eries [Rr]ule {refer_oRuleName} was [Pp]rocessed for the TargetSeries {refer_oTargetSeries}={refer_oTargetSeries}.addSeriesRuleProcessed({refer_oRuleName});
...
[consequence][][Rr]ecord that the [Dd]ose [Rr]ule {refer_oRuleName} was [Pp]rocessed for the TargetDose {refer_oTargetDose}={refer_oTargetDose}.addDoseRuleProcessed({refer_oRuleName});
...
[consequence][][Pp]rint [Dd]ebug [Mm]essage about [Oo]bject [Nn]amed {strObjectName} and [Vv]alue {oDebugObject}=System.out.println("[DEBUG RULE: " + drools.getRule().getName() + "] " + {strObjectName} + " = " + {oDebugObject});
[consequence][][Pp]rint [Dd]ebug [Mm]essage {strMessage}=System.out.println("[DEBUG RULE: " + drools.getRule().getName() + "] " + {strMessage});
